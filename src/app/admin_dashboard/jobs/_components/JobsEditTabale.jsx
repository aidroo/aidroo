...
export default function JobsEditDialog({ id, currentVerified, currentStatus }) {
  ...
  const handleUpdate = async () => {
    setLoading(true);
    try {
      const response = await axiosInstance.put(`/api/jobs/${id}`, {
        id,
        verified,
        status,
      });
      ...
      if (response?.data?.status === 201) {
        ...
        router.refresh("/admin_dashboard/jobs");
      }
    } catch (error) {
